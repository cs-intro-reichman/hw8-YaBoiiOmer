 ...
 *  a list of user names that s/he follows, and the list's size. */
 public class User {
    ...
    public boolean addFollowee(String name) {
        ...
        if(follows(name) || this.name == name) return false;
        follows[fCount] = name;
        fCount++;
        return true;
    }
    ...
    public boolean removeFollowee(String name) {
        int removedIndex = -1;
        for(int i = 0; i < fCount; i++){
            if(!follows[i].equals(name)) continue;
            follows[i] = null;
            removedIndex = i;
            break;
        }
        ...
        for (int i = removedIndex; i < fCount -1; i++){
    ...
    public int countMutual(User other) {
        ...
        for(int j = 0; j < other.getfCount(); j++){
            if(this.follows(other.getfFollows()[j])) intersection += 1;
        }
        return intersection;
    }
